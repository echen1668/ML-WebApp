...
exp_names = db.models.distinct("exp_name", {"type": "AutoGluon", "is_FS": True})
...
def test_model(models, test_data_raw, outcomes, train_data_raw=None, cutoff_index='youden'):
    ...
    for outcome in outcomes:
        ...
        if outcome in list(models.keys()):
            input_columns = models[outcome]['Selected Features']
            print(f'Input Columns: {input_columns}')
        else:
            print("No model for the current outcome.")
            continue
        ...
        try:
            ...
            feature_importance_df = model.feature_importance(test_data)
...
if model_type != 'AutoGluon' or exp_dic['is_FS'] == False:
    st.write("Results is not AutoGluon and/or not Feature Selection.")
    exp_dic = None
    model_path = None
    model_type = None
...
if model_path is not None and uploaded_test_set is not None and len(selected_outcomes)!=0:
    ...
    if st.button('Test the models'):
        ...
        try:
            ...
            results_dictonary, outcome_dic, feature_importance_dic = test_model(models_dic, test_set, selected_outcomes, train_data_raw=train_set, cutoff_index=threshold_type)
